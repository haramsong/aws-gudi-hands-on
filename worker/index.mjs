...
2. 둘째 줄: 이 문제가 발생하면 어떤 결과가 초래되는지 한 줄로 간략히 설명 (한국어)
3. 수정 필요 시: 빈 줄 후 suggestion 블록 추가
4. suggestion 블록은 RIGHT side 줄에만 사용 가능

카테고리 이모지:
  🐛 버그/오류 (오타, 런타임 에러, 컴파일 에러, 잘못된 값 포함)  🔒 보안 이슈  ⚡ 성능 개선  🧹 코드 스타일 (포맷팅, 네이밍 컨벤션, 가독성)  💡 제안  ✅ 좋은 코드
